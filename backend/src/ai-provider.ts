...
export interface AIProvider {
  ...
  analyze(input: { organizationId: string; userId?: string; message: string }): Promise<AIProviderResult>;
}
...
async function loadKnowledge(organizationId: string, message: string, userId?: string): Promise<KnowledgeContext> {
...
class KnowledgeFallbackProvider implements AIProvider {
  ...
  async analyze(input: { organizationId: string; userId?: string; message: string }): Promise<AIProviderResult> {
    const knowledge = await loadKnowledge(input.organizationId, input.message, input.userId);
...
abstract class HttpProvider implements AIProvider {
  ...
  protected async prompt(input: { organizationId: string; userId?: string; message: string }) {
    const knowledge = await loadKnowledge(input.organizationId, input.message, input.userId);
  ...
  abstract analyze(input: { organizationId: string; userId?: string; message: string }): Promise<AIProviderResult>;
}
