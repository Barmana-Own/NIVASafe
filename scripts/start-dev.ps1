...
if (-not (Test-Path "package.json") -or -not (Test-Path "backend/prisma/schema.prisma")) {
...
# Prisma CLI runs from backend in this pnpm workspace and does not reliably load
...
Write-Host "Uploads:  backend\uploads" -ForegroundColor Green
Write-Host ""
