...
const backendText = readFiles(path.join(root, "backend/src"), ".ts").map((file) => fs.readFileSync(file, "utf8")).join("\n");
const frontendText = readFiles(path.join(root, "frontend/src"), ".tsx").concat(readFiles(path.join(root, "frontend/src"), ".ts")).map((file) => fs.readFileSync(file, "utf8")).join("\n");
