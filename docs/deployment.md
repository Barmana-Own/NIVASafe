...
در روش PM2، `DATABASE_URL` معمولاً به MySQL محلی یا سرور دیتابیس اشاره می‌کند، مثلاً `127.0.0.1:3306`. پوشه `apps/web/dist` را با Nginx سرو کنید و درخواست‌های `/api/` را به `127.0.0.1:5044` Proxy کنید. قانون SPA باید تمام مسیرهای ناشناخته را به `index.html` برگرداند.
...
```bash
SMOKE_API_URL=https://hse.example.com/api/v1 \
