...
function buildLocalRulaSuggestions(analysis: RulaPostureAnalysis, inputs: RulaInput, bodySide: RulaActionBodySide = "RIGHT"): RulaCorrectionAction[] {
  ...
  return suggestions.length ? suggestions.map((suggestion) => ({ ...suggestion, bodySide, source: "FALLBACK" as const })) : buildLocalRulaFallbackSuggestions(bodySide);
}
...
function RulaReportView({ result, analysis, activityInfo, factors, sideResults, sideFactors, suggestions: rawSuggestions, selectedActions, bodySide, actionBodySide, onActionBodySideChange, locale, busyActionId, onToggleAction, onAddAction, onRemoveAction, suggestionsLoading = false, suggestionsStatus, suggestionsError, onRetrySuggestions, showResultHero = true }: { result: { score: number; actionLevel: number; explanation?: string; status?: string }; analysis: RulaPostureAnalysis; activityInfo?: RulaActivityInfo | null; factors: RulaReportFactor[]; sideResults?: Partial<Record<RulaBodySide, RulaSideAssessmentResult>>; sideFactors?: Partial<Record<RulaBodySide, RulaReportFactor[]>>; suggestions: RulaCorrectionAction[]; selectedActions: RulaCorrectionAction[]; bodySide: RulaActionBodySide; actionBodySide: RulaActionBodySide; onActionBodySideChange?: (side: RulaActionBodySide) => void; locale: "fa" | "en"; busyActionId?: string | null; onToggleAction?: (action: RulaCorrectionAction) => void; onAddAction?: (action: RulaCorrectionAction) => void; onRemoveAction?: (action: RulaCorrectionAction) => void; suggestionsLoading?: boolean; suggestionsStatus?: ReportActionAiStatus; suggestionsError?: string; onRetrySuggestions?: () => void; showResultHero?: boolean }) {
  ...
  const predictedScore = predictedRulaScoreForSide(visibleResult.score, selectedActions, showBothSides ? activeReportSide : undefined);
  ...
  return <div className="rula-report-view">
    ...
    {showBothSides && <><div className="rula-side-tabs rula-report-side-tabs" role="tablist" aria-label={t("assessment.bodySide")}>{(["RIGHT", "LEFT"] as const).map((side) => <button type="button" role="tab" aria-selected={activeReportSide === side} className={activeReportSide === side ? "active" : ""} onClick={() => setActiveReportSide(side)} key={side}>{side === "RIGHT" ? t("assessment.right") : t("assessment.left")}</button>)}</div>{sideResults?.RIGHT && sideResults.LEFT && <div className="rula-side-score-strip" aria-label={t("assessment.bothSides")}><span><small>{t("assessment.right")}</small><strong>{sideResults.RIGHT.score.toLocaleString(numberLocale)}</strong></span><span><small>{t("assessment.left")}</small><strong>{sideResults.LEFT.score.toLocaleString(numberLocale)}</strong></span><span className="final"><small>{t("assessment.bothSides")}</small><strong>{result.score.toLocaleString(numberLocale)}</strong></span></div>}</>}    <div className={`rula-report-score-card ${actionLevel.className}`}><div className="rula-report-score"><div className="rula-report-score-label"><small>{t("assessment.rulaScoreLabel")}</small><span className="rula-report-score-icon" aria-hidden="true"><Icon name="rula" size={21}/></span></div><strong>{visibleResult.score.toLocaleString(numberLocale)}</strong><span>{t("assessment.rulaCurrentScoreDescription")}</span></div><div className="rula-report-risk"><div className="rula-report-risk-head"><small>{t("assessment.rulaRiskLevel")}</small><span className={`rula-report-risk-badge ${actionLevel.className}`} role="status">{t(actionLevel.labelKey)}</span></div><div className="rula-report-status-line"><span>{t("assessment.rulaAssessmentStatus")}</span><StatusBadge value={result.status ?? "DRAFT"}/></div><p>{mainFactor ? t("assessment.rulaMainFactorSummary", { factor: t(`assessment.${mainFactor.key}`) }) : t("assessment.rulaSummaryText")}</p></div></div>
...
export function RulaReportPage() {
  ...
  const suggestions = useMemo(() => {
    ...
    if (loadedSuggestions.length || !report) return loadedSuggestions;
    return buildLocalRulaSuggestionsForAssessment(report.assessment.postureAnalysis, rulaInputsFromAnalysis(report.assessment.postureAnalysis, 0, false), report.assessment.bodySide);
