...
function FmeaReportStepper({ onStepClick }: { onStepClick?: (step: FmeaWizardStep) => void }) {
  const { t } = useI18n();
  const steps: Array<{ step: FmeaWizardStep; number: string; label: string }> = [
    { step: 1, number: "02", label: t("report.processStep") },
    { step: 2, number: "03", label: t("report.analysisStep") },
    { step: 3, number: "04", label: t("report.finalStep") },
  ];
  return <nav className="fmea-report-stepper" aria-label={t("assessment.stepsLabel", { type: "FMEA" })}>{steps.map(({ step, number, label }) => {
    const clickable = Boolean(onStepClick && step !== 3);
    return <button type="button" key={step} aria-current={step === 3 ? "step" : undefined} className={step === 3 ? "current" : "done"} onClick={clickable ? () => onStepClick?.(step) : undefined} disabled={!clickable}><b>{number}</b><span>{label}</span></button>;
  })}</nav>;
}
...
function FmeaReviewRiskRow({ draft, rows, context, autoEnabled, onChange, onScoreChange, onAdd, onRemove, onAccept, onAutoAccept, onAcceptScore, onAutoAcceptScore }: { draft: FmeaRiskRowInput; rows: FmeaRiskRowInput[]; context: Omit<FmeaRiskSuggestionContext, "failureMode" | "effect" | "cause" | "recommendation">; autoEnabled: boolean; onChange: <K extends keyof FmeaRiskRowInput>(key: K, value: FmeaRiskRowInput[K]) => void; onScoreChange: (kind: FmeaScoreKind, value: number) => void; onAdd: () => void; onRemove: (index: number) => void; onAccept: (field: FmeaRiskSuggestionField, value: string) => void; onAutoAccept: (field: FmeaRiskSuggestionField, value: string) => boolean; onAcceptScore: (suggestion: FmeaRiskScoreSuggestion) => void; onAutoAcceptScore: (suggestion: FmeaRiskScoreSuggestion) => boolean }) {
  ...
  return <div className="fmea-review-risk-card" aria-labelledby="fmea-review-risk-title">
    ...
    <div className="form-grid three">
      <label>{t("assessment.failureMode")} <span className="required-label">{t("common.required")}</span><input name="reviewFailureMode" value={draft.failureMode} placeholder={t("assessment.failureModePlaceholder")} onChange={(event) => onChange("failureMode", event.target.value)}/></label>
      <label>{t("assessment.effect")} <span className="required-label">{t("common.required")}</span><input name="reviewEffect" value={draft.effect} placeholder={t("assessment.effectPlaceholder")} onChange={(event) => onChange("effect", event.target.value)}/></label>
      <label>{t("assessment.cause")} <span className="required-label">{t("common.required")}</span><input name="reviewCause" value={draft.cause} placeholder={t("assessment.causePlaceholder")} onChange={(event) => onChange("cause", event.target.value)}/></label>
      <label>{t("assessment.preventiveControls")} <span className="optional-label">{t("common.optional")}</span><input name="reviewPreventiveControls" value={draft.preventiveControls} placeholder={t("assessment.existingControls")} onChange={(event) => onChange("preventiveControls", event.target.value)}/></label>
      <label>{t("assessment.detectionControls")} <span className="optional-label">{t("common.optional")}</span><input name="reviewDetectionControls" value={draft.detectionControls} placeholder={t("assessment.detectionPlaceholder")} onChange={(event) => onChange("detectionControls", event.target.value)}/></label>
      <label className="span-two">{t("assessment.recommendation")} <span className="optional-label">{t("common.optional")}</span><textarea name="reviewRecommendation" rows={2} value={draft.recommendation} placeholder={t("assessment.recommendationPlaceholder")} onChange={(event) => onChange("recommendation", event.target.value)}/></label>
    </div>
...
function FmeaProcessPage() {
  ...
  return <section className="page-shell">
    {registeredAssessmentsView ? <PageHeader eyebrow={t("assessment.fmeaEyebrow")} title={t("assessment.registered")} description={t("assessment.registeredDescription")} actions={<button type="button" className="ghost button-link" onClick={() => navigate("/fmea")}><Icon name="arrow" className="back-arrow" size={16}/>{t("assessment.backToNewFmea")}</button>}/> : <PageHeader eyebrow={t("assessment.fmeaEyebrow")} title={t("assessment.fmeaTitle")} description={t("assessment.fmeaDescription")} actions={<div className="page-actions-inline">{canEdit() && <button type="button" className={`fmea-assistant-toggle ${fmeaAssistantEnabled ? "enabled" : "disabled"}`} aria-pressed={fmeaAssistantEnabled} onClick={() => setFmeaAssistantEnabled((enabled) => !enabled)}><span className="fmea-assistant-toggle-mark" aria-hidden="true">{fmeaAssistantEnabled ? "✓" : ""}</span><span><strong>{t("assessment.fmeaAssistant")}</strong><small>{fmeaAssistantEnabled ? t("assessment.fmeaAssistantOn") : t("assessment.fmeaAssistantOff")}</small></span></button>}<button type="button" className="ghost button-link fmea-registered-button" aria-controls="fmea-registered-assessments" aria-expanded={registeredAssessmentsView} onClick={openRegisteredAssessments}><Icon name="fmea" size={16}/>{t("assessment.registered")}</button><Link className="ghost button-link" to="/choose-path"><Icon name="arrow" className="back-arrow" size={16}/>{t("assessment.changeAssessmentMethod")}</Link></div>}/>}
    ...
    {registeredAssessmentsView && selectedAssessment && <SectionCard title={t("assessment.riskRows", { title: selectedAssessment.title })} description={t("assessment.riskRowsDescription")} icon="chart" actions={canEdit() ? <div className="risk-report-actions"><button type="button" className="primary" onClick={() => navigate(`/fmea/${selectedAssessment.id}/report`)}><Icon name="chart" size={15}/> {t("assessment.openReport")}</button><button type="button" className="ghost" onClick={() => downloadFmeaReport("xlsx")}><Icon name="download" size={15}/> Excel</button><button type="button" className="ghost" onClick={() => downloadFmeaReport("pdf")}><Icon name="download" size={15}/> PDF</button><button type="button" className="ghost" onClick={() => downloadFmeaReport("docx")}><Icon name="download" size={15}/> Word</button></div> : undefined}>
      ...
      {registeredAssessmentsView && selectedAssessment && canEdit() && wizardStep === 2 && <SectionCard title={t("assessment.addRiskRow")} description={t("assessment.scoreDescription")} icon="plus"><AutoSaveForm id="fmea-risk-row-form" storageKey={itemDraftKey} className="fmea-item-form" onSubmit={addItem}><div className="form-grid three"><label>{t("assessment.failureMode")} <span className="required-label">{t("common.required")}</span><input name="failureMode" placeholder={t("assessment.failureModePlaceholder")} required/></label><label>{t("assessment.effect")} <span className="required-label">{t("common.required")}</span><input name="effect" placeholder={t("assessment.effectPlaceholder")} required/></label><label>{t("assessment.cause")} <span className="required-label">{t("common.required")}</span><input name="cause" placeholder={t("assessment.causePlaceholder")} required/></label><label>{t("assessment.preventiveControls")} <span className="optional-label">{t("common.optional")}</span><input name="preventiveControls" placeholder={t("assessment.existingControls")}/></label><label>{t("assessment.detectionControls")} <span className="optional-label">{t("common.optional")}</span><input name="detectionControls" placeholder={t("assessment.detectionPlaceholder")}/></label><label className="span-two">{t("assessment.recommendation")} <span className="optional-label">{t("common.optional")}</span><textarea name="recommendation" rows={2} placeholder={t("assessment.recommendationPlaceholder")}/></label></div><div className="score-panel"><div className="score-panel-fields"><FmeaScoreField kind="severity" name="severity" value={scores.severity} onChange={(value) => { scoreTouchedRef.current = true; setScores((current) => ({ ...current, severity: value })); }}/><span aria-hidden="true">×</span><FmeaScoreField kind="occurrence" name="occurrence" value={scores.occurrence} onChange={(value) => { scoreTouchedRef.current = true; setScores((current) => ({ ...current, occurrence: value })); }}/><span aria-hidden="true">×</span><FmeaScoreField kind="detection" name="detection" value={scores.detection} onChange={(value) => { scoreTouchedRef.current = true; setScores((current) => ({ ...current, detection: value })); }}/></div><div className="score-panel-actions"><div className="rpn-preview" aria-live="polite"><div className="rpn-preview-copy"><small>{t("assessment.calculatedRpn")}</small><strong>{previewRpn.toLocaleString(numberLocale)}</strong></div><StatusBadge value={previewRiskLevel}/></div><button className="primary" type="submit" disabled={itemCreating}><Icon name="plus"/> {itemCreating ? t("assessment.savingChanges") : t("assessment.calculateRegister")}</button></div></div><FmeaRiskAiAssist autoRequestKey={fmeaAssistantEnabled ? [locale, selectedAssessment.id, selectedAssessment.items.length, selectedAssessment.title, selectedAssessment.department ?? "", selectedAssessment.activityDescription ?? ""].join("|") : ""} getContext={riskRowFormContext} onAccept={applyRiskSuggestionToForm} onAutoAccept={(field, value) => applyRiskSuggestionToForm(field, value, true)} onAcceptScore={(suggestion) => { scoreTouchedRef.current = true; setScores({ severity: suggestion.severity, occurrence: suggestion.occurrence, detection: suggestion.detection }); }} onAutoAcceptScore={(suggestion) => { if (scoreTouchedRef.current) return false; scoreTouchedRef.current = true; setScores({ severity: suggestion.severity, occurrence: suggestion.occurrence, detection: suggestion.detection }); return true; }}/></AutoSaveForm></SectionCard>}
  </section>;
}
...
function RulaMuscleUseSelector({ value, expanded, locale, onToggle, onChange }: { value: boolean; expanded: boolean; locale: "fa" | "en"; onToggle: () => void; onChange: (value: boolean) => void }) {
  const { t } = useI18n();
  const score = value ? 1 : 0;
  const numberLocale = locale === "en" ? "en-US" : "fa-IR";
  return <section className="rula-muscle-use-field"><button type="button" className={`rula-muscle-use-trigger ${expanded ? "expanded" : ""}`} aria-expanded={expanded} aria-controls="rula-muscle-use-options" onClick={onToggle}><span className={`rula-muscle-use-mark ${value ? "active" : ""}`} aria-hidden="true">{value ? <Icon name="check" size={13}/> : <span>+</span>}</span><span className="rula-muscle-use-trigger-copy"><strong>{t("assessment.repetitiveMuscle")}</strong><small>{t("assessment.repetitiveMuscleHint")}</small></span><span className="rula-muscle-use-selected-score">{t("assessment.repetitiveMuscleSelectedScore", { score: score.toLocaleString(numberLocale) })}</span><span className="rula-muscle-use-chevron" aria-hidden="true">⌄</span></button>{expanded && <div id="rula-muscle-use-options" className="rula-muscle-use-options" role="radiogroup" aria-label={t("assessment.repetitiveMuscle")}>{<label className={`rula-muscle-use-option ${value ? "selected" : ""}`}><input type="radio" name="muscleUseOption" value="1" checked={value} onChange={() => onChange(true)}/><span><strong>{t("assessment.repetitiveMuscleCriterionOne")} · {t("assessment.repetitiveMuscleCriterionOneScore")}</strong><small>{t("assessment.repetitiveMuscleCriterionOneHint")}</small></span></label>}<label className={`rula-muscle-use-option ${!value ? "selected" : ""}`}><input type="radio" name="muscleUseOption" value="0" checked={!value} onChange={() => onChange(false)}/><span><strong>{t("assessment.repetitiveMuscleCriterionZero")} · {t("assessment.repetitiveMuscleCriterionZeroScore")}</strong><small>{t("assessment.repetitiveMuscleCriterionZeroHint")}</small></span></label></div>}</section>;
}
...
function RulaReportView({ result, analysis, activityInfo, factors, suggestions: rawSuggestions, selectedActions, bodySide, actionBodySide, onActionBodySideChange, locale, busyActionId, onToggleAction, onAddAction, onRemoveAction, showResultHero = true }: { result: { score: number; actionLevel: number; explanation?: string; status?: string }; analysis: RulaPostureAnalysis; activityInfo?: RulaActivityInfo | null; factors: RulaReportFactor[]; suggestions: RulaCorrectionAction[]; selectedActions: RulaCorrectionAction[]; bodySide: RulaActionBodySide; actionBodySide: RulaActionBodySide; onActionBodySideChange?: (side: RulaActionBodySide) => void; locale: "fa" | "en"; busyActionId?: string | null; onToggleAction?: (action: RulaCorrectionAction) => void; onAddAction?: (action: RulaCorrectionAction) => void; onRemoveAction?: (action: RulaCorrectionAction) => void; showResultHero?: boolean }) {
  ...
  return <div className="rula-report-view">
    ...
    <nav className="rula-result-stepper" aria-label={t("assessment.rulaResultSteps")}><span className="complete"><b>۱</b><small>{t("assessment.processInformation")}</small></span><i aria-hidden="true"/><span className="complete"><b>۲</b><small>{t("assessment.rulaResultStepPosture")}</small></span><i aria-hidden="true"/><span className="active"><b>۳</b><small>{t("assessment.rulaResultStepOutcome")}</small></span><i aria-hidden="true"/><span><b>۴</b><small>{t("assessment.rulaResultStepFinalReport")}</small></span></nav>
             ...
             <div className="rula-process-fields">
               ...
               <div className="rula-process-form-grid">
                 <label className="span-two">{t("assessment.projectRequired")} <span className="required-label">{t("common.required")}</span><StyledSelect name="projectId" value={selectedProjectId} onChange={(event) => { const value = event.target.value; if (value === RULA_CREATE_PROJECT_OPTION) { openRulaProjectCreator(); return; } setSelectedProjectId(value); }} required><option value="">{t("assessment.projectSelect")}</option>{projects.data?.map((project) => <option key={project.id} value={project.id}>{projectName(project, locale)}</option>)}<option value={RULA_CREATE_PROJECT_OPTION}>＋ {t("projects.newProject")}</option></StyledSelect></label>
                  ...
                  <label className="span-two">{t("assessment.rulaJobTitle")} <span className="required-label">{t("common.required")}</span><input name="jobTitle" maxLength={180} defaultValue={draftValue(draft, "jobTitle")} placeholder={t("assessment.rulaJobTitlePlaceholder")} required/></label>
                  <label className="span-two">{t("assessment.rulaTask")} <span className="required-label">{t("common.required")}</span><textarea name="taskDescription" maxLength={500} rows={2} defaultValue={draftValue(draft, "taskDescription")} placeholder={t("assessment.rulaTaskPlaceholder")} required/></label>
                 ...
                 <label>{t("assessment.rulaLoadWeight")} <span className="optional-label">{t("common.optional")}</span><span className="measure-control"><input name="loadWeight" type="number" min="0" max="10000" step="0.1" defaultValue={draftValue(draft, "loadWeight")} placeholder="0"/><StyledSelect name="loadUnit" defaultValue={draftValue(draft, "loadUnit", "KG")}><option value="KG">{t("assessment.kilograms")}</option><option value="LB">{t("assessment.pounds")}</option></StyledSelect></span></label>
                 <label>{t("assessment.subjectCode")} <span className="optional-label">{t("common.optional")}</span><input name="subjectCode" defaultValue={draftValue(draft, "subjectCode")} placeholder={t("assessment.subjectCodePlaceholder")}/></label>
               </div>
             </div>
